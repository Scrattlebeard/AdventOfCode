...
fun setupChallenge(): Challenge<List<Pair<Pair<String, Int>, List<String>>>> {
    return setup {
        day(16)
        year(2022)
...
fun findOptimalPaths(
    ...
    (closedValves.indices).forEach {
        if (time.first > 1) {
            val options = closedValves.toMutableList()
            val distanceBound = options.minBy { distances[paths.first.last()]!![it.first]!! }
            val choice1 = options.removeAt(it)
            if (choice1.second >= distanceBound.second) {
                val cost1 = distances[paths.first.last()]!![choice1.first]
    ...
    (closedValves.indices).forEach {
        if (time.second > 1) {
            ...
            if (choice2.second >= distanceBound.second) {
                val cost2 = distances[paths.second.last()]!![choice2.first]
    ...
    (closedValves.indices).forEach {
        if (time.first > 1 && time.second > 1 && closedValves.size > 1) {
            val options = closedValves.toMutableList()
            val distanceBound1 = options.minBy { distances[paths.first.last()]!![it.first]!! }
            val choice1 = options.removeAt(it)
            ...
            if (choice1.second >= distanceBound1.second) {
                ...
                if (cost1 != null && cost1 <= time.first - 1) {
                    ...
                    options.indices.forEach {
                        ...
                        val distanceBound2 =
                            options.minBy { distances[paths.first.last()]!![it.first]!! }
